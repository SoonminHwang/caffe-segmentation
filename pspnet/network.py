...
def build_context_large(net, bottom, num_classes, layers=8):
    prev_layer = bottom
    multiplier = 2
    for i in range(1, 3):
        conv_name = 'ctx_conv1_{}'.format(i)
...
def make_image_label_data_bn(image_list_path, label_list_path, batch_size,
                          mirror, crop_size, mean_pixel,
                          label_stride=8, margin=186):
    label_dim = (crop_size - margin * 2) // 8
    data, label = L.ImageLabelData(
    ...
    return data_scale, label


def build_frontend_vgg19_bn(net, bottom, num_classes):
    prev_layer = bottom
    # num_convolutions = [2, 2, 3, 3, 3]        # VGG16
    num_convolutions = [2, 2, 4, 4, 4]
    dilations = [0, 0, 0, 0, 2, 4]
    for l in range(5):
        num_outputs = min(64 * 2 ** l, 512)
        for i in range(0, num_convolutions[l]):
            conv_name = 'conv{0}_{1}'.format(l+1, i+1)
            relu_name = 'relu{0}_{1}'.format(l+1, i+1)
            bn_name = 'bn{0}_{1}'.format(l+1, i+1)
            if dilations[l] == 0:
                setattr(net, conv_name,
                        L.Convolution(
                            prev_layer,
                ...
                setattr(net, conv_name,
                        L.Convolution(
                            ...
                            convolution_param=dict(num_output=num_outputs,
                                                   ...
                                                   dilation=dilations[l])))
            setattr(net, bn_name,
                    L.BatchNorm(getattr(net, conv_name), 
                        param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0)],
                        batch_norm_param=dict(moving_average_fraction=0.9)))
            setattr(net, relu_name,
                    L.ReLU(getattr(net, bn_name), in_place=True))
            prev_layer = getattr(net, relu_name)
    ...
    net.fc6 = L.Convolution(
        ...
        convolution_param=dict(num_output=4096, kernel_size=7,
                               dilation=dilations[5]))
    net.fc6_bn = L.BatchNorm(
        net.fc6,
        param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0)],
        batch_norm_param=dict(moving_average_fraction=0.9))
    net.relu6 = L.ReLU(net.fc6_bn, in_place=True)
    # net.drop6 = L.Dropout(net.relu6, in_place=True, dropout_ratio=0.5)
    net.drop6 = L.Dropout(net.relu6, in_place=True, dropout_ratio=0.7)
    ...
    net.fc7 = L.Convolution(
        ...
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)],
        convolution_param=dict(num_output=4096, kernel_size=1))
    net.fc7_bn = L.BatchNorm(
        net.fc7,
        param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0)],
        batch_norm_param=dict(moving_average_fraction=0.9))
    net.relu7 = L.ReLU(net.fc7_bn, in_place=True)
    # net.drop7 = L.Dropout(net.relu7, in_place=True, dropout_ratio=0.5)
    net.drop7 = L.Dropout(net.relu7, in_place=True, dropout_ratio=0.7)
    net.final = L.Convolution(
        net.drop7,
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)],
        convolution_param=dict(
...
def build_context_large_bn(net, bottom, num_classes, layers=8):
    prev_layer = bottom
    multiplier = 2
    for i in range(1, 3):
        conv_name = 'ctx_conv1_{}'.format(i)
        relu_name = 'ctx_relu1_{}'.format(i)
        bn_name = 'ctx_bn1_{}'.format(i)
        setattr(net, conv_name,
                L.Convolution(
                    *([] if prev_layer is None else [prev_layer]),
                    param=[dict(lr_mult=1, decay_mult=1),
        ...
        setattr(net, bn_name,
                    L.BatchNorm(getattr(net, conv_name), 
                        param=[dict(lr_mult=1, decay_mult=0), dict(lr_mult=1, decay_mult=0), dict(lr_mult=1, decay_mult=0)],
                        batch_norm_param=dict(moving_average_fraction=0.9)))
        setattr(net, relu_name,
                L.ReLU(getattr(net, bn_name), in_place=True))
        prev_layer = getattr(net, relu_name)
    ...
    net.ctx_fc1 = L.Convolution(
        ...
        convolution_param=dict(
            ...
            weight_filler=dict(type='identity',
                               ...
                               std=0.01 / multiplier),
            bias_filler=dict(type='constant', value=0)))
    net.ctx_fc1_bn = L.BatchNorm(
        net.ctx_fc1,
        param=[dict(lr_mult=1, decay_mult=0), dict(lr_mult=1, decay_mult=0), dict(lr_mult=1, decay_mult=0)],
        batch_norm_param=dict(moving_average_fraction=0.9))
    net.ctx_fc1_relu = L.ReLU(net.ctx_fc1_bn, in_place=True)
